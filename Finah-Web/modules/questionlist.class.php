...
class QuestionList {
    ...
    function __construct($connection) {
        if ($connection != 'api') {
            ...
            // HAAL OP VIA WEP API  //
            $json = file_get_contents('http://finah-webapi-appdevit03.azurewebsites.net/api/questionlist/');
            $qL = json_decode($json, TRUE);
            $json = file_get_contents('http://finah-webapi-appdevit03.azurewebsites.net/api/question/');
            $qQ = json_decode($json, TRUE);
            $json = file_get_contents('http://finah-webapi-appdevit03.azurewebsites.net/api/theme/');
